...
export default function TryOnModal({
  ...
  const processTryOn = useCallback(async () => {
    ...
    try {
      ...
      if (msg.includes("429") || msg.includes("Too Many")) {
        setErrorMsg(
          "⏳ Hệ thống AI đang quá tải. Vui lòng đợi 30 giây rồi thử lại.",
        );
      } else if (msg.includes("500")) {
        setErrorMsg(
          "AI đang gặp lỗi. Vui lòng kiểm tra Gemini API key trong Cài đặt Admin.",
        );
      } else if (msg.includes("422") || msg.includes("không thể")) {
        setErrorMsg(
          "AI không thể tạo ảnh thử kính với ảnh này. Vui lòng thử ảnh khuôn mặt khác.",
        );
      } else {
  ...
  const fallbackCanvasProcess = async () => {
    ...
    ctx.fillRect(
      canvas.width / 2 - wmMetrics.width / 2 - 16,
      wmY - wmSize,
      wmMetrics.width + 32,
      wmSize * 2.2,
    );
    ...
    img.onload = () => {
      ...
      const drawWatermarkAndSave = (logoImage?: HTMLImageElement) => {
        ...
        for (
          let y = -canvas.height;
          y < canvas.height;
          y += Math.max(80, canvas.height * 0.15)
        ) {
          for (
            let x = -canvas.width;
            x < canvas.width;
            x += Math.max(200, canvas.width * 0.4)
          ) {
      ...
      <div className="tryon-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="tryon-modal__colors">
              {product.colors.map((c, i) => (
                <button
                  ...
                  style={{
                    backgroundColor: c === "transparent" ? "#f5f5dc" : c,
                  }}
        ...
        <div className="tryon-modal__body">
          ...
          {showCamera && (
            <div className="tryon-modal__camera">
              ...
              <div className="tryon-modal__camera-actions">
                <button
                  className="tryon-modal__cam-cancel"
                  onClick={stopCamera}
                >
                  <FiX /> Hủy
                </button>
                <button
                  className="tryon-modal__cam-capture"
          ...
          {faceImage && !processing && !resultImage && (
            <div className="tryon-modal__preview">
              <div className="tryon-modal__preview-img">
                ...
                <button
                  className="tryon-modal__preview-remove"
          ...
          {errorMsg && !processing && !resultImage && (
            <div
              className="tryon-modal__error tryon-fade-in"
              style={{
                textAlign: "center",
                padding: "32px 20px",
              }}
